// WebSocket handlers for Lumeris DApp - Real-time updates and notifications
...
const setupWebSocketHandlers = (ws) => {
  ...
  ws.send(
    JSON.stringify({
      type: "connection",
      message: "Connected to Lumeris DApp WebSocket server",
  ...
  const mockNotifications = [
    {
      ...
      message: "You earned 150 Lumeris tokens from Crypto Warriors!",
      timestamp: new Date().toISOString(),
      read: false,
    },
    {
      ...
      message: "Lumeris-ETH pool now offers 48.5% APY!",
      timestamp: new Date().toISOString(),
      read: false,
    },
  ];
